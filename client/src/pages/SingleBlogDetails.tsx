...
const SingleBlogDetails: React.FC = () => {
    const { blog, category } = useParams<{ blog: string; category: string }>()

    const { data, loading, error } = useFetch<Data>(`${getEvn('VITE_API_BASE_URL')}/blog/get-blog/${blog}`, {
        method: 'get',
        credentials: 'include',
    }, [blog, category])
            ...
            {data && data.blog &&
                <>
                    <div className='border rounded md:w-[70%] w-full p-5'>
                        ...
                        <div dangerouslySetInnerHTML={{ __html: decode(data.blog.blogContent) || '' }}></div>
